...
mode = st.sidebar.radio(
    "モードを選んでください",
    ("🏠 ホーム", "📷 画像アップロードで推論", "⚔️ AIと予測対戦", "📊 戦績表示")
)
...
# --------------------------------------------------
if mode == "🏠 ホーム":
    st.title("🌍 GeoGuessアプリへようこそ！")
    st.markdown("""
    ### 🎯 このアプリについて
    景色が **どの国で撮影されたのかをAIが予測** します。  
    ViT（Vision Transformer）と呼ばれるAIモデルベースに学習した独自モデルを使用しています。

    ### 🧩 モード紹介
    - **📷 画像アップロードで推論**  
      あなたがアップロードした写真に対して、AIが撮影国を予測します。  
      各ブロックごとのアテンションマップや、ViTの累積アテンション（Rollout）を可視化します。

    - **⚔️ AIと予測対戦**  
      ランダムに表示される景色を見て、あなたが国を予想します。  
      AIと同じ景色で推論を行い、どちらが正解に近いか勝負！

    - **📊 戦績表示**  
      これまでの対戦記録を確認できます。  
      あなたとAIの正答率を比較し、勝率を分析できます。

    ### 📡 技術構成
    本アプリは **クラウド3層アーキテクチャ** を採用しています：
    - **フロントエンド:** Streamlit  
    - **バックエンド:** FastAPI  
    - **クラウドストレージ:** Google Cloud Storage  
    - **データ:** OpenStreetView-5M (CVPR 2024)

    ### 🚀 遊び方
    サイドバーから好きなモードを選択して体験してください！
    """)
